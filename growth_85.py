...
#dataframe#
df = pd.read_csv("/Users/matsuda_teppei/Documents/Nm6/122/WT_85.csv")
...
df = pd.read_csv("/Users/matsuda_teppei/Documents/Nm6/122/dTS_85.csv")
df1 = df.groupby("conc",as_index = False,sort = False).describe()["dpm"]
df2 = df.query("replicates == 'rep1'")
df1["conc"] = df2["conc"]
df3,cov = curve_fit(IC50,df["conc"],df["dpm"], p0=[  0.62057893, -11.05923388,   7.12642261,   0.09113962])
xs, ys = graph_plot(df1["conc"].max()+1,df3[0],df3[1],df3[2],df3[3])
df4 = pd.DataFrame({'conc': xs, 'dpm': ys})
print(df3)
...
x2 = df["conc"]
y2 = df["dpm"]
ax.errorbar(x1,y1,yerr = yerr,ecolor = "#FF0000",color = "#FF0000",capsize = 10,fmt = "s",markersize=10,capthick = 1,elinewidth = 1,zorder=1)



df = pd.read_csv("/Users/matsuda_teppei/Documents/Nm6/122/dTS_TS_85.csv")
df1 = df.groupby("conc",as_index = False,sort = False).describe()["dpm"]
df2 = df.query("replicates == 'rep1'")
df1["conc"] = df2["conc"]
df3,cov = curve_fit(IC50,df["conc"],df["dpm"], p0=[ 0.61774467, -8.3835741,   6.98319237,  0.08505468])
xs, ys = graph_plot(df1["conc"].max()+1,df3[0],df3[1],df3[2],df3[3])
df4 = pd.DataFrame({'conc': xs, 'dpm': ys})
print(df3)
...
y = df4["dpm"]
yerr = df1["std"]
ax.plot(x,y,color = "#3333FF",lw =1.5,zorder=2)
x1 = df1["conc"]
y1 = df1["mean"]
x2 = df["conc"]
y2 = df["dpm"]
ax.errorbar(x1,y1,yerr = yerr,ecolor = "#3333FF",color = "#3333FF",capsize = 10,fmt = "^",markersize=10,capthick = 1,elinewidth = 1,zorder=2)
